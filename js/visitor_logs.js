...
function setupSidebarAndTabs() {
  ...
  links.forEach(btn => {
    btn.addEventListener("click", function () {
      ...
      // ✅ 추가된 로그 조회 탭 진입 시 자동 로드 및 초기 날짜 세팅 로직
      if (targetId === "visitorLogSection") {
        ...
        loadDeptVisitorLogs(); // 탭 열 때 자동 조회
      }
  ...
  if (user && user.userName) {
    ...
    if (logButton) {
      if (user.level === 3 || user.level === 2) {
        ...
        // ✅ [수정됨] 페이지 리다이렉트(href 이동) 제거 (탭 방식으로 작동하도록)
      } else {
...
function loadDeptVisitorLogs() {
  ...
  getData(url, (logs) => {
    ...
    logs.forEach(row => {
      ...
      // 값이 변경되었는지 확인하여 강조 효과
      const isChanged = (beforeB !== afterB) || (beforeL !== afterL) || (beforeD !== afterD);

      const tr = document.createElement("tr");
      
      // formatToKoreanTime 함수 사용 (util.js 에 존재)
      const timeStr = typeof formatToKoreanTime === "function" ? formatToKoreanTime(row.updated_at) : row.updated_at;

      tr.innerHTML = `
        <td>${row.date}</td>
        <td>${row.dept || "-"}</td>
        <td style="font-weight:bold;">${row.applicant_name}</td>
        <td style="color:#666;">${beforeStr}</td>
        <td style="color:${isChanged ? '#2563eb' : '#28a745'}; font-weight:bold;">${afterStr}</td>
        <td style="font-size:0.85rem; color:#888;">${timeStr}</td>
      `;
